...
public class DefaultPopulationInitializationStrategy implements
	...
	public DefaultPopulationInitializationStrategy(IEvolutionStrategy strategy) {
	...
	public void initialize(IPopulation pop, IGenome genome, int size) {
		...
		for (int i = 1; i <= size; i++) {
			...
			strategy.getMutationStrategy().mutateLinkWeight(newgenome,1.0, 1.0, MutationType.GAUSSIAN);
		...
		strategy.getSpeciationStrategy().speciate(pop);
	}
