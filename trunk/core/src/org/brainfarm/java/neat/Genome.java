...
public class Genome implements IGenome {
	...
	public IGenome mateMultipoint(IGenome g, int id, double fitness1, double fitness2) {
	...
	public IGenome mateMultipointAverage(IGenome g, int id, double fitness1, double fitness2) {
   ...
   *
   */
	public IGenome mateSinglepoint(IGenome spouse, int id) {
		//String mask4 = " 0000";
		...
		int len_traits = traits.size();
		int size1 = genes.size();
		int size2 = spouse.getGenes().size();
		int crosspoint = 0;
		//int len_genome = 0;
		...
		for (j = 0; j < len_traits; j++) {
			_trait1 = traits.get(j);
			_trait2 = spouse.getTraits().get(j);
			newtrait = new Trait(_trait1, _trait2);
			newtraits.add(newtrait);
		...
		if (size1 < size2) {
			...
			//len_genome = size2;
			genomeA = genes;
			genomeB = spouse.getGenes();
		} else {
			crosspoint = RandomUtils.randomInt(0, size2 - 1);
			stopA = size2;
			stopB = size1;
			//len_genome = size1;
			genomeA = spouse.getGenes();
			genomeB = genes;
		}
	...
	public void mutateRandomTrait() {
		...
		// Choose a random traitnum
		int traitnum = RandomUtils.randomInt(0, traits.size() - 1);
		// Retrieve the trait and mutate it
		traits.get(traitnum).mutate();
	...
	public boolean mutateAddNode(IPopulation population) {
